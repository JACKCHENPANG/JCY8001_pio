...
static uint8_t autorange_pick(long long re, long long im, uint16_t vzm) {
    ...
    /* 低内阻电芯: 10Ω 探测档对 sub-mΩ 信号很弱, s2 读出≈0 或很小. 旧逻辑 s2<=0 默认回 10Ω,
     * 恰恰把最该用大电流档的低阻电芯锁在了最不灵敏的档. 改: |Z| < 3mΩ (s2 < (3000μΩ)²=9e6)
     * 直接用最灵敏的 1Ω 档(大电流、信号强、分辨率高), 无论探得准不准都会落到对的档. */
    if (s2 < 9.0e6) return 2;                                  /* |Z| < 3mΩ → 1Ω(~1mΩ) 最灵敏档 */
