...
volatile uint16_t jcy_aux_temp = 0x8000;  // 0x3301 U8 GuardDieTemp (T*10,有符号); 0x8000=无效
volatile uint16_t jcy_voltage;       // 0x3340 U8测量芯片主电压 (V*10000)
volatile uint16_t jcy_aux_voltage;   // 0x3341 U8 GuardVolt (V*10000)
...
static void init_registers(void) {
    ...
    jcy_fw_version = 0x0231;   // v2.31: expose U8 GuardDieTemp(0x3301) + GuardVolt(0x3341)
    jcy_git_rev    = 0x0001;
...
static uint16_t get_reg(uint16_t addr) {
    ...
    switch (addr) {
        ...
        case 0x3301: return jcy_aux_temp;    // 辅助温度: U8 GuardDieTemp
        ...
        case 0x3341: return jcy_aux_voltage; // 辅助电压: U8 GuardVolt
...
#define DNB_DATA_GUARDVOLT    0x01
#define DNB_DATA_MAINDIETEMP  0x04
#define DNB_DATA_GUARDDIETEMP 0x05
...
static void delay_us(uint32_t us){ uint32_t t0=DWT->CYCCNT, n=us*8u, g=0; while((DWT->CYCCNT-t0)<n){ if(++g>4000000u) break; } }  // CPU=8MHz: 8cyc/µs; g保险防DWT不计数时死循环
static inline uint32_t dwt_ms_since(uint32_t t0){ return g_ms - t0; }  // t0=g_ms快照; SysTick 1ms tick
...
int main(void)
{
    ...
    uint16_t zm_conv_target = 100;  // (旧)轮询周期门 — 仅留作超时保护
    uint32_t zm_start_cyc = 0;      // ZM 启动时刻 (DWT CYCCNT), 真实时间门用
    uint32_t zm_conv_ms = 1100;     // 本次 ZM 应等的真实毫秒数 (按频率 exp 自适应)
    ...
    while (1) {
        ...
        if (usart2_rx_pop(&c)) {
            ...
            if (dnb_ic_count > 0 && measure_ticks >= MEASURE_INTERVAL) {
                ...
                // U8辅助: GuardDieTemp/GuardVolt 暴露给上位机 0x3301/0x3341。
                uint32_t aut = dnb_get_data(DNB_MEAS_ID, DNB_DATA_GUARDDIETEMP);
                int16_t at12 = dnb_decode_main_die_temp(aut);
                jcy_aux_temp = (uint16_t)((int32_t)at12 * 5 / 8);
                dnb_delay_cycles(8000);
                uint32_t auv = dnb_get_data(DNB_MEAS_ID, DNB_DATA_GUARDVOLT);
                ...
                // ds18b20_poll();  // 暂停: 探头未接 + delay_us(DWT忙等)在CYCCNT不计数时会死循环; 待DWT确认+探头接好再开
                ...
                if (zm_start_req) {
                    ...
                    // ── 转换门: 改用 DWT 真实时间(不再数轮询周期, 否则被BLE负载拖成抖动+over-wait) ──
                    // 原厂 ConvTime=max(1100, 1050*2^(7-exp)) ms。低频(exp小)转换久。
                    // 等真实 ConvTime×余量: 标准1.3x / 快速1.0x。DWT@8MHz, 不受蓝牙轮询影响 → 每点时间确定、且砍掉旧版~2x over-wait。
                    {
                        uint8_t ze = (uint8_t)((jcy_zm_freq_set >> 8) & 0x0F);
                        uint32_t conv_ms = (ze >= 7) ? 1100u : (1050u << (7 - ze));
                        zm_conv_ms = jcy_zm_fast ? conv_ms : (conv_ms * 130u / 100u);
                        if (zm_conv_ms < 300u) zm_conv_ms = 300u;
                        if (jcy_zm_convovr) zm_conv_ms = jcy_zm_convovr;   // 调试覆盖(ms)
                        zm_start_cyc = g_ms;   // g_ms 快照 (SysTick ms)
                        zm_conv_target = 60000u;   // 轮询周期硬超时保护(远大于真实门, 仅防DWT异常死循环)
                    }
                } else if (zm_running) {
                    dnb_delay_cycles(8000);
                    ...
                    // 完成条件: BalZMDone提前置位 | 真实时间到(DWT, 主判据) | 轮询硬超时(防DWT异常)
                    if ((flags & DNB_SRVREQ_BALZMDONE) || (dwt_ms_since(zm_start_cyc) >= zm_conv_ms) || (++zm_cycles >= zm_conv_target)) {
